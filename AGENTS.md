...
| `tests/DotnetSourceResolver.Core.Tests` | Unit tests (54) + live tests (3) |
| `tests/DotnetSourceResolver.McpServer.Tests` | In-process MCP integration tests (6) |
...
3. `GitHubAdapter` — shared raw-file fetch primitive used by the adapters above; also an `ISourceAdapter` for pre-resolved GitHub URLs
...
- `design.md` lists `RESOLVER_CACHE_DIR` and `RESOLVER_ENABLE_DECOMPILATION` — **neither is implemented**.
